...
/**
 * Compatibility port numbers for RP2354.
 *
 * RP2354 does not have LPC-style GPIO ports. For source compatibility we map:
 *   GPIO0..31  -> PIO0_0..PIO0_31
 *   GPIO32..47 -> PIO1_0..PIO1_15
 *
 * PIO2/PIO3 are kept only so existing code that refers to the enum names still
 * compiles, but there are no physical GPIOs behind them on RP2354.
 */
enum Port
{
    PIO0 = 0,
 ...
 *
 * The values are kept compatible with the existing sblib API so PINMODE_FUNC()
 * continues to work unchanged in application code.
 */
enum PinFunc
{
 ...
 *
 * Example:
 *   pinMode(PIO1_6, INPUT | PINMODE_FUNC(PF_RXD));
 */
#define PINMODE_FUNC(f) ((f) << 18)

/*
 * Keep the existing function-field layout so digital_pin.cpp can continue to
 * extract the requested function from the mode value in the same way.
 */
enum
{
    PFL_ADMODE = 0x100,
...
/**
 * RP2354-compatible PortPin encoding.
 *
 * Bits 0..4  : pin number within the compatibility port
 * Bits 5..6  : compatibility port number
 * Bits 9..29 : optional function metadata, kept for API compatibility
 *
 * Unlike the LPC11xx branch, the function metadata is not used to describe
 * actual silicon routing possibilities. It is only present so existing sblib
 * macros and mode handling keep compiling.
 */
enum PortPin
{
    /* GPIO 0..31 -> compatibility port 0 */
    PIO0_0  = 0x00,
    PIO0_1  = 0x01,
    PIO0_2  = 0x02,
    PIO0_3  = 0x03,
    ...
    /* GPIO 32..47 -> compatibility port 1 */
    PIO1_0  = 0x20,
    PIO1_1  = 0x21,
    PIO1_2  = 0x22,
    PIO1_3  = 0x23,
 ...
 * Analog channels.
 *
 * RP2354 does not provide LPC11xx-compatible ADC channel/pin mapping in sblib.
 * The enum is kept only for source compatibility for now.
 */
enum AnalogChannel
{
    AD0 = 0,
 ...
 *
 * Kept for source compatibility. RP2354 does not use LPC masked-access ports.
 */
 ...
 *
 * On RP2354 this is a compatibility helper only. It returns:
 *   0 for PF_PIO
 *   1 for any explicitly requested alternate function
 *  -1 if the function is unsupported for the given pin
 */
